"""
Modified code from https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/gaussian_diffusion.py
"""
...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        """
        fac = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x.shape)
    ...
    def get_eps(
        ...
        if cond_fn is not None:
            ...
            # print(f'alpha: {(th.sqrt(1 - alpha_bar)).shape}')
            # print(f'cond: {(cond_fn(x, t, **model_kwargs)).shape}')
            eps = eps - th.sqrt(1 - alpha_bar) * cond_fn(x, t, **model_kwargs)
            
            # print(f'after eps: {eps.shape}')
        return eps
